...
class ska2_pk(Likelihood):
	...
	self.th_error_flag = False
        if (self.theoretical_error>0.):
		self.th_error_flag = True
	if (self.use_halofit or self.th_error_flag):
            	self.need_cosmo_arguments(data, {'non linear':'halofit'})
		print("Using halofit")


	# Print warning for combining theo. error and NL cutoff
	if (self.kmax==0 and self.th_error_flag):
	...
	self.zmax -= self.dz/2.
        ...
        # At the center of each bin, compute the biais function,
        # using fit fromula from 1412.4700v2: c4*e^c5z
        self.b = self.c4*np.exp(self.c5*self.z_mean)
	...
	# Then V_fid and the fiducial errors on real space coordinates follow.
	...
	if self.th_error_flag :
        	...
        	# uncertainty.
		# Delta chi^2 = 1 normalization is commented
        ...
        # recover the e_th part of the neutrino error function
