...
class ska2_pk(Likelihood):
	...
	if 'beta_0^SKA2' in self.use_nuisance:
        	b = self.c4*np.exp(self.c5*self.z_mean*data.mcmc_parameters['beta_1^SKA2']['current']*data.mcmc_parameters['beta_1^SKA2']['scale'])*data.mcmc_parameters['beta_0^SKA2']['current']*data.mcmc_parameters['beta_0^SKA2']['scale']
	else:
	...
	if self.use_zscaling==0:
        ...
        if 'beta_0^SKA2' in self.use_nuisance:
            chi2 += ((data.mcmc_parameters['beta_0^SKA2']['current']*data.mcmc_parameters['beta_0^SKA2']['scale']-1.)/self.bias_accuracy)**2
            chi2 += ((data.mcmc_parameters['beta_1^SKA2']['current']*data.mcmc_parameters['beta_1^SKA2']['scale']-1.)/self.bias_accuracy)**2
